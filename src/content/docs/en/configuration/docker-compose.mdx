...
> The generated Compose file uses GHCR images by default. When the CLI is run in local-build mode, it builds the MCP server from `packages/mcp-server/Dockerfile` and the Telegram bot from `packages/mcp-telegram-client/Dockerfile` inside the monorepo.
