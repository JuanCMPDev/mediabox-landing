...
- **Lista de origenes permitidos** -- protege endpoints MCP/API invocables desde navegador usando los origenes configurados
- **Sandbox de rutas y URLs** -- rechaza rutas con traversal y destinos de descarga inseguros/privados antes de ejecutar herramientas
- **Tokens de confirmacion** -- tokens de un solo uso emitidos por el servidor para forzar ejecucion destructiva en dos pasos
- **Endpoint de salud** -- `/health` devuelve el estado del servidor

### Dominios de Herramientas

El servidor MCP proporciona **30 herramientas** organizadas en **6 dominios**:
...
| **Library** | 5 | Jellyfin, sistema de archivos, Sonarr, Radarr | Gestion de biblioteca, operaciones de archivos, renombrado de episodios, snapshots de estado y correccion de subtitulos |
| **Sonarr** | 7 | Sonarr | Busqueda de series, monitoreo, lanzamientos, descargas, importaciones y reescaneos |
| **Radarr** | 7 | Radarr | Busqueda de peliculas, monitoreo, lanzamientos, descargas, importaciones y reescaneos |
...
Las operaciones de larga duracion (movimiento de archivos, optimizacion de medios, conversion de subtitulos) se ejecutan como trabajos en segundo plano. El servidor MCP informa su estado a traves de la herramienta `check_jobs`. Cada trabajo tiene ID, estado, mensaje, hora de inicio y resultado opcional; algunas respuestas de inicio tambien incluyen una duracion estimada.
...
## Hardening de Seguridad 2.2

La version 2.2 movio varias protecciones desde instrucciones de prompt hacia enforcement del lado servidor:

- Los checks de origen de navegador protegen `/mcp`, `/api/dashboard`, `/api/chat` y `/api/setup`.
- Las rutas de archivos y carpetas quedan aisladas bajo las raices de media y descargas; se rechaza traversal con `..`.
- Las URLs de descarga se validan contra esquemas inseguros y destinos de red privada antes de entrar a la cola.
- Las herramientas destructivas devuelven primero preview y `confirmToken`; solo ejecutan cuando se repiten los mismos argumentos con ese token dentro de 5 minutos.
